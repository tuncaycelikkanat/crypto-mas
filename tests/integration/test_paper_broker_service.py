...
def test_paper_broker_executes_target_portfolio(db_session: Session) -> None:
    ...
    assert report.ending_cash == pytest.approx(4999.0, abs=0.1)
    assert report.starting_equity == 10000.0
    assert report.ending_equity == pytest.approx(9998.0, abs=0.1)
    ...
    assert float(account.cash_balance) == pytest.approx(4999.0, abs=0.1)
    assert float(account.equity) == pytest.approx(9998.0, abs=0.1)
